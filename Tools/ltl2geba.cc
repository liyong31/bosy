...
void ltlf2geba(const char *ltlf, const char *outs, bool product)
{
    ...
    }else 
    {
        ...
        spot::twa_graph_ptr nba_left = trans.run(fprime);
        spot::twa_graph_ptr nba_right = trans.run(formula::Not(f));
        nba = spot::product(nba_left, nba_right);
    }
...
int main(int argc, char **argv)
{
    ...
    if (argc > 3)
    {
        ltlf2geba(argv[1], argv[2], false);
    }
    else
    {
        ltlf2geba(argv[1], argv[2], true);
    }
